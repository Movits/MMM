...
export default function IntelligentMatches() {
  ...
  return <main className="min-h-screen bg-transparent px-4 py-8 text-white md:px-8">
    <div className="mx-auto max-w-6xl">
      <button onClick={() => navigate("/dashboard")} className="mb-6 inline-flex items-center gap-2 text-sm text-white/55 hover:text-white"><ArrowLeft size={16}/> Dashboard</button>
      <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-end">
        ...
        <button disabled={recalculate.isPending} onClick={() => recalculate.mutate()} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#f5a623] px-5 py-3 font-bold text-[#08121f] disabled:opacity-50"><RefreshCw size={18} className={recalculate.isPending ? "animate-spin" : ""}/> Atualizar matches</button>
      </div>
