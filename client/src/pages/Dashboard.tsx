                ...
                return (
                  <section key={grupo.chave} className="space-y-3">
                    ...
                    {doGrupo.map((conn, i) => {
                      ...
                      // O id da conexão só vem do servidor no pedido que ESTA pessoa
                      // responde (revisão da #115: o id é sequencial e, no clique
                      // recíproco, o número sozinho contava quem pediu primeiro). Nos
                      // outros cartões ele é null, e é por isso que os botões saem
                      // deste const e não de `esperaPorMim` sozinho: sem o id não há o
                      // que responder, e o cartão mostra o selo de status.
                      const idParaResponder = esperaPorMim ? conn.id : null;
                      ...
                      return (
                        // A chave é a posição dentro do grupo, e não o id: o servidor
                        // só manda o id da conexão no pedido que esta pessoa responde
                        // (o id é sequencial e contava quem clicou primeiro — revisão
                        // da #115). Com id nulo na maioria das linhas, uma chave tirada
                        // dele colidiria entre os cartões.
                        <div key={i}
                          style={{
                            opacity: 1,
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            {idParaResponder !== null ? (
                              <>
                                <button onClick={() => respondMutation.mutate({ connectionId: idParaResponder, accept: true })}
                                ...
                                <button onClick={() => respondMutation.mutate({ connectionId: idParaResponder, accept: false })}
