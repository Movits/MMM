...
describe("o prompt do LLM recebe rótulos em português", () => {
  it("(3, 6) busca, valores, especialidade, setor, idiomas e interesses saem traduzidos", async () => {
    ...
    expect(prompt).toContain(`- Busca: ${ptBR.onboarding.seeking.strategic_partner}, ${ptBR.onboarding.seeking.investor}`);
...
describe("o motor oficial é um só", () => {
  ...
  it("profileMatches.ts só expõe: não pontua nem escreve score", () => {
    ...
    // Repassar a nota gravada (`overallScore: m.overallScore`) é expor, não pontuar.
    expect(router).not.toMatch(/overallScore\s*[:=](?!\s*m\.overallScore\b)/);
  });
