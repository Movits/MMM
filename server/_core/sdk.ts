...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    // If user not in DB:
    // - For email+senha auth (openId starts with 'email_'): user must exist, never sync via OAuth
    // - For Manus OAuth: sync from OAuth server automatically
    if (!user) {
      if (sessionUserId.startsWith("email_")) {
        // Auth próprio: usuário não encontrado no banco — sessão inválida
        console.error("[Auth] Email auth user not found in DB:", sessionUserId);
